...
export class FeUserService {
	...
	private loggedUser: BehaviorSubject<User | null> =
		new BehaviorSubject<User | null>(this.tryToRecoverUserFromStorage());
	constructor(
	...
	getAllUsers(): Observable<User[]> {
		return this.http
			...
			.pipe(
				map((res: User[]) => res.map((user: User) => new User(user)))
			);
	}
	...
	login(data: CreateUserDto): void {
		...
		this.http.post<User>(`${this.apiUrl}/login`, data).subscribe((x) => {
			if (x) {
				this._snackBar.open("User logged successfully", "", {
					duration: 5000,
					panelClass: ["snackbar-success"]
				});
				this.loggedUser.next(new User(x));
	...
	logout(): void {
		...
		this.http.post<void>(`${this.apiUrl}/logout`, {}).subscribe(() => {
			this._snackBar.open("User logged out successfully", "", {
				duration: 5000,
				panelClass: ["snackbar-success"]
			});
	...
	private isUserLoggedIn(altMessage: string | null = null): boolean {
		if (this.loggedUser.value) {
			this._snackBar.open(
				altMessage ? altMessage : "User already logged in",
				"",
