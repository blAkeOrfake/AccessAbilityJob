...
export class OffersComponent implements OnInit {
	...
	selectedSortOption: { optionLabel: string; optionValue: string } | null =
		null;
	...
	get filteredOffers(): JobOffer[] {
		const filtered = this.offers.slice(
			this.paginationState.first,
			(this.paginationState.first ?? 0) + (this.paginationState.rows ?? 0)
		);
		const filteredByName = this.searchInput
			? filtered.filter((offer) =>
					offer.title
						.toLowerCase()
						.includes(this.searchInput?.toLowerCase() ?? "")
			  )
			: filtered;
		const sorted = this.selectedSortOption
			? filteredByName.sort(this.selectedSortFn)
			: filteredByName;
		return sorted;
	}
	...
	private refreshJobOffers(): void {
		this.jobOfferService.getAllJobOffers().subscribe((offers) => {
			console.log(offers);
			this.offers = offers;
			this.paginationState.pageCount = Math.ceil(
				this.offers.length / (this.paginationState?.rows ?? 10)
			);
		});
	}
	...
	onSortChange(ev: DropdownChangeEvent): void {
		...
		switch (ev.value.optionValue) {
			case "newest":
				this.selectedSortFn = (a, b) =>
					b.postedDate.getTime() - a.postedDate.getTime();
				break;
			case "oldest":
				this.selectedSortFn = (a, b) =>
					a.postedDate.getTime() - b.postedDate.getTime();
				break;
			case "highest":
				this.selectedSortFn = (a, b) =>
					b.salaryRange.max - a.salaryRange.max;
				break;
			case "lowest":
				this.selectedSortFn = (a, b) =>
