...
export class ApplicationsComponent {
  ...
  get filteredApplications(): JobApplication[] {
    return this.jobApplications.filter((application) => application.userId === this.currentUser?.id);
  }
  ...
  getApplications(): void {
    this.jobApplicationService.getAllJobApplications().subscribe((applications) => {
  ...
  acceptApplication(jobApplication: JobApplication): void {
    this.jobApplicationService.updateJobApplication(jobApplication.id, { ...jobApplication, status: 'Accepted' }).subscribe(() => {
      this.getApplications();
    });
  }

  denyApplication(jobApplication: JobApplication): void {
    this.jobApplicationService.updateJobApplication(jobApplication.id, { ...jobApplication, status: 'Denied' }).subscribe(() => {
      this.getApplications();
    });
  }
}
