  ...
  onSubmit() {
    if (this.jobOfferForm.valid) {
      ...
      this.offerService.createJobOffer(jobOfferData).subscribe(
        () => {
          this._snackBar.open("Job offer added successfully", 'Go to details', { duration: 5000 });
        }, (error) => {
          console.error(error);
      ...
      // Handle the job offer data (e.g., send it to a server)
    }
  }
  ...
  // Randomly generate a job offer
    ...
    fillWithRandomData() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const randomRequirement = new FormControl(this.getRandomOption(jobOfferDetails.requirements));
        requirements.push(randomRequirement);
      }

      const responsibilities = this.jobOfferForm.get('responsibilities') as FormArray;
      responsibilities.clear();
      for (let i = 0; i < 3; i++) {
        const randomResponsibility = new FormControl(this.getRandomOption(jobOfferDetails.responsibilities));
      ...
      benefits.clear();
      for (let i = 0; i < 3; i++) {
        const randomBenefit = new FormControl(this.getRandomOption(jobOfferDetails.benefits));
        benefits.push(randomBenefit);
      }

      const accessibilityFeatures = this.jobOfferForm.get('accessibilityFeatures') as FormArray;
      accessibilityFeatures.clear();
      for (let i = 0; i < 3; i++) {
        const randomAccessibilityFeature = new FormControl(this.getRandomOption(jobOfferDetails.accessibilityFeatures));
