...
export class FeJobApplicationService {
  ...
  createJobApplication(jobApplication: Omit<JobApplication, 'id'>): Observable<JobApplication> {
    return this.http.post<JobApplication>(this.apiUrl, jobApplication).pipe(
      tap(() => this.showSnackBar('Job application created successfully')),
      catchError(error => {
  ...
  getJobApplicationById(id: string): Observable<JobApplication> {
    return this.http.get<JobApplication>(`${this.apiUrl}/${id}`).pipe(
      tap(() => this.showSnackBar('Job application retrieved successfully')),
      catchError(error => {
  ...
  updateJobApplication(id: string, jobApplication: Omit<JobApplication, 'id'>): Observable<JobApplication> {
    return this.http.put<JobApplication>(`${this.apiUrl}/${id}`, jobApplication).pipe(
      tap(() => this.showSnackBar('Job application updated successfully')),
      catchError(error => {
  ...
  deleteJobApplication(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/${id}`).pipe(
      tap(() => this.showSnackBar('Job application deleted successfully')),
      catchError(error => {
